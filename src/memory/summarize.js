 ...
 */
export async function maybeSummarize(userId, channelId) {
  const db = getDb();
  const thread = db.prepare('SELECT id, messages, summary FROM threads WHERE user_id = ? AND channel_id = ?').get(userId, channelId);
  if (!thread) return;
